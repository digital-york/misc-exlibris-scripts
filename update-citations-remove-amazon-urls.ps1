# PEH Sept April 2019
# PS script to remove amazon urls from citations
...
#loop to get values and store it
	for ($i=1; $i -le $rowMax-1; $i++)
	{
		...
		Write-Host "Processing row number" ($rowCourse+$i) 
		...
		#this block removes source node if it is an amazon url
		
		$str_source = $xml.selectSingleNode('//metadata/source[contains(text(),"amazon")]/text()').Value
		
		
		Write-Host 'source is ' $str_source
		
		if (!$str_source){
			Write-Host "Non-Amazon URL."
		 }else{
		        Write-Host 'amazon URL'
				$parent_xpath = '//metadata'
				$nodes = $xml.SelectNodes($parent_xpath)
				$nodes | % {
						$child_node = $_.SelectSingleNode('source')
						$_.RemoveChild($child_node) | Out-Null
						$xml.save("C:\Work\Alma\scripts\citation-xml.txt")
						}
				#update citation call
					try{
						Invoke-WebRequest -Method 'PUT' -Uri $full_call -Body $xml -ContentType application/xml 
							...
							write-host "Fatal error: Update Citation" -ForegroundColor Red
							write-host "Exception Type: $($_.Exception.GetType().FullName)" -ForegroundColor Red
							write-host "Exception Message: $($_.Exception.Message)" -ForegroundColor Red	
							Add-Content "C:\Work\Alma\Scripts\Errors\put-errors.txt" $citation_id
						}				
					}
